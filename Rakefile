...
class Generator
  ...
  def build
    %x(xcodebuild -workspace #{$name}.xcworkspace -scheme #{$name} -configuration Release -sdk iphoneos7.1 -derivedDataPath . clean)
    %x(xcodebuild -workspace #{$name}.xcworkspace -scheme #{$name} -configuration Release -sdk iphoneos7.1 -derivedDataPath . CODE_SIGN_IDENTITY=\"#{$signature}\")
  end
  def generateIpa
    %x(xcrun -sdk iphoneos PackageApplication -v #{$appPath} -o #{$ipaFile} -sign \"#{$signature}\" -embed #{$provisionProfile})
...
task :tag do |t|
  puts `defaults write #{$infoFile} CFBundleShortVersionString 0.2`
end
